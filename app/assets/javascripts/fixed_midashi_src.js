...
function _createFixedHeader(tblSource, opt, tableCount)
{
	...
	for (var i = 0; i < opts.length; i++) {
		...
		case "class": clazz = value; break; // (ver1.10)
...
//	}
	...
	return new _FixedHeader(divSource, tblSource, columnCount,
		...
		copyId, clazz, borderColor, borderStyle, borderWidth, boxShadow, bgcolor, opacity, bodyHeader, leftHeader);
}
...
function _FixedHeader(_divSource, _tblSource, _columnCount,
	...
	_copyId, _clazz, _borderColor, _borderStyle, _borderWidth, _boxShadow, _bgcolor, _opacity, _bodyHeader, _leftHeader)
{
...
// テーブルコピー
	...
	function _createTable(tid, rows, cols)
	{
		...
		_linkElement(table, _tblSource, tid, _copyId, _clazz, false);
		if (caption != null) _linkElement(caption, srcCaption, tid, _copyId, _clazz, true);
		if (thead != null) _linkElement(thead, srcThead, tid, _copyId, _clazz, false);
		if (tbody != null) _linkElement(tbody, srcTbody, tid, _copyId, _clazz, false);
		...
		if (_isDivMode) {
			...
			_linkElement(div, _divSource, tid, _copyId, _clazz, false);
	...
	function _copyCells(table, tbody, thead, tid, rows, cols, theadRows)
	{
		...
		for (var row = 0; row < rows; row++) { // 行のループ
			var srcTr = srcTrList[row];
			var dstTr = srcTr.cloneNode(false);
			_linkElement(dstTr, srcTr, tid, _copyId, _clazz, false);
			...
			for (var col = 0; col < cols; ) { // 列のループ
				...
				_linkElement(dstTd, srcTd, tid, _copyId, _clazz, true);
				dstTr.appendChild(dstTd);
 ...
 */
function _linkElement(dst, src, tid, copyId, clazz, linkChild)
{
	if (src.id && !copyId) {
		dst.removeAttribute("id");
	}
	if (clazz) {
		var curClass = dst.getAttribute("class");
		if (curClass) {
			dst.setAttribute("class", curClass + " " + clazz);
		} else {
			dst.setAttribute("class", clazz);
		}
	}
	if (src.name) {
	...
	if (linkChild) { // 子ノードも
		for (var i = 0; i < dst.childNodes.length; i++) {
			...
			_linkElement(child, src.childNodes[i], tid, copyId, clazz, linkChild); // 再帰
		}
	}
}
...
function _setEventHandler(dst, src)
{
	...
	if ((src.tagName == "INPUT") ||
		...
		case "password":
		case "textarea":
		default: // html5 の type に対応。(2018/12 ver1.11)
			...
			dst.onkeyup = function() {
				_copyValue(dst, src);  // dst の値を src にコピー
				_copyValues(src, dst); // src の値を他の dst にコピー
				if (src.onkeyup) return src.onkeyup();
				return true;
			}; // ver1.11
			if (src.$FXH_ON_CHANGE_FUNC == undefined) {
				...
				_addEventListener(src, "keyup", src.$FXH_ON_CHANGE_FUNC); // ver1.11
			}
			break;
...
*/
	// chrome で仕様が変わったため修正。(2017/10 ver1.10)
